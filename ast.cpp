...
{
    // std::cout << "literalNode " << "type " << static_cast<int>(type) << " value " << value << std::endl;
    switch (type)
    {
    case Type::Int:
        return "int";
...
std::string BinaryExprNode::analyze(SymbolTable &symbols) const
{
    ...
    //  std::cout << "binary " << leftType << " " << rightType << std::endl;
...
std::string
BuiltinCallNode::analyze(SymbolTable &symbols) const
{
